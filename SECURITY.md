...
- `npm audit --omit=dev --audit-level=high` runs in `prepublishOnly` and
  ...
  0 critical, 4 moderate (logged to `docs/v0.9.3/backlog.md` for triage).
...
Both workflows (`ci.yml`, `publish.yml`) pin every third-party action
to a commit SHA, not a floating tag. Workflow-scope `permissions:` is
locked to `contents: read` by default. The publish workflow adds
`id-token: write` at job scope (required by `npm publish --provenance`
OIDC attestation). No `pull_request_target` triggers anywhere — fork-
escalation surface is empty.
...
This document covers magpie-mcp **v0.9.2** (audit landed 2026-05-14).
See `docs/v0.9.2/audit-01-security.md` for the per-probe result table
